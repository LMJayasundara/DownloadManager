...
function getMetadata(videoFilePath) {
...
const addMetadata = (filePath, videoId, type, format, status, url) => {
      ...
      ytdl.getInfo(url, { requestOptions: { headers: { "Cookie": data.cookie } } }).then(info => {
        ...
        ffmpegProcess.on('exit', (code, signal) => {
          ...
          if (code === 0) {
            ...
            addMetadata(outputFilePath, videoId, "mp3", 'youtube', "COMPLETE", url)
          } else {
            console.log(`FFmpeg exited with code ${code} and signal ${signal}`);
          }
          clearDownloadProgress(downloadProgress, videoId);
...
rl.on('line', async (input) => {
  ...
  switch (input.trim()) {
    ...
    case '4':
      console.log(await getMetadata('./videos/5ChkQKUzDCs.mp3'))
      break;
    case '5':
